const mpbuttonBox = document.createElement('template');


mpbuttonBox.innerHTML = `
  <style>
  .MaypayButtonBox{
    display: flex;
    ...
    .importo{
        font-family: "__MPMavenM", sans-serif;
        font-size: 13px;
    }
    .info{
        text-align: center;
        font-size: 16px;
        line-height: 21px;
        font-family: "__MPMaven", sans-serif;
        letter-spacing: 0px;
        color: #606060;
        opacity: 1;
    }
...
<div class="MaypayButtonBox">
  <div class="importo">Totale: <span class="valore"></span> €</div>
  <div class="logo"><img src="${env.pluginsUrl}/maypay/assets/images/maypay_logo_180.png" alt="logo Maypay"/></div>
  <div class="info">
  <p>E se fosse gratis?</p>
  <p>Maypay è l’app per vincere</p>
  <p>ciò che stai acquistando.</p>
  </div>
...
class MaypayOnlineButtonBox extends HTMLElement {
    constructor() {
        super();

        this._shadowRoot = this.attachShadow({ 'mode': 'closed' });
        this._shadowRoot.appendChild(mpbuttonBox.content.cloneNode(true));
    }
    ...
    get paymentRequestAmount() {
        return this.getAttribute('paymentRequestAmount');
    }
    ...
    connectedCallback() {
        ...
        this.$amount = this._shadowRoot.querySelector('.valore');
        this.$amount.textContent = this.paymentRequestAmount;
...
window.customElements.define('maypay-online-button-box', MaypayOnlineButtonBox);